      ...
      <div className="rounded-3xl glass-card border border-white/15 p-6 sm:p-8 shadow-2xl relative min-h-[480px] flex flex-col justify-between">
        ...
        {currentStep === 1 && (
          <div className="space-y-5 animate-in fade-in duration-200">
            ...
            {/* Instant AI Auto-Inferred Compatibility Card (No Manual Entry Required) */}
            <div className="p-4 sm:p-5 rounded-2xl bg-gradient-to-r from-cyan-950/40 via-slate-900 to-amber-950/30 border border-cyan-500/30 space-y-3">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <h4 className="text-sm font-bold text-white">
                      Auto-Configured Profile for <span className="text-amber-400">{selectedFoodObj.name}</span>
                    </h4>
                  </div>
                </div>

                {/* Instant 1-Click Recommendation CTA */}
                <button
                  type="button"
                  onClick={handleStartAnalysis}
                  className="px-5 py-2.5 rounded-xl text-xs font-extrabold bg-gradient-to-r from-amber-500 via-amber-400 to-cyan-400 hover:from-amber-400 hover:to-cyan-300 text-slate-950 flex items-center justify-center gap-2 shadow-lg shadow-amber-500/25 transition-all transform hover:scale-[1.02] cursor-pointer"
                >
                  <Zap className="w-4 h-4 fill-current text-slate-950" />
                  <span>Instant 1-Click AI Recommendation</span>
                </button>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 pt-1 text-center text-xs">
                <div className="p-2 rounded-xl bg-black/40 border border-white/5">
                  <span className="text-[9px] text-slate-400 font-mono uppercase block">Moisture / Aw</span>
                  <span className="font-bold text-cyan-300 font-mono">{selectedFoodObj.moistureContent} ({selectedFoodObj.waterActivity})</span>
                </div>
                <div className="p-2 rounded-xl bg-black/40 border border-white/5">
                  <span className="text-[9px] text-slate-400 font-mono uppercase block">Respiration</span>
                  <span className="font-bold text-amber-300 font-mono">{selectedFoodObj.respirationRate}</span>
                </div>
                <div className="p-2 rounded-xl bg-black/40 border border-white/5">
                  <span className="text-[9px] text-slate-400 font-mono uppercase block">Ideal Cold Temp</span>
                  <span className="font-bold text-emerald-300 font-mono">{selectedFoodObj.idealTempC}°C</span>
                </div>
                <div className="p-2 rounded-xl bg-black/40 border border-white/5">
                  <span className="text-[9px] text-slate-400 font-mono uppercase block">Max Shelf Life</span>
                  <span className="font-bold text-white font-mono">{selectedFoodObj.maxShelfLifePackagedDays} Days</span>
                </div>
                <div className="p-2 rounded-xl bg-black/40 border border-white/5 col-span-2 sm:col-span-1">
                  <span className="text-[9px] text-slate-400 font-mono uppercase block">Primary Risk</span>
                  <span className="font-bold text-rose-300 text-[11px] truncate block">{selectedFoodObj.keyRisks[0]}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          {currentStep === 1 ? (
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={handleStartAnalysis}
                className="px-4 py-2 rounded-xl text-xs font-bold bg-gradient-to-r from-amber-500 to-cyan-400 text-slate-950 flex items-center gap-1.5 shadow-md shadow-amber-500/25 transition-all transform hover:scale-[1.02]"
              >
                <Zap className="w-3.5 h-3.5 fill-current" /> Instant AI Match
              </button>
              <button
                onClick={() => setCurrentStep(2)}
                className="px-4 py-2 rounded-xl text-xs font-bold bg-white/10 hover:bg-white/20 text-white flex items-center gap-1.5 transition-all"
              >
                Fine-Tune <ChevronRight className="w-4 h-4" />
              </button>
            </div>
          ) : currentStep < 5 ? (
            <button
